module AlphaVantageRb
  module HelperFunctions
    def method_missing(method, *args, &block)
      raise AlphaVantageRb::Error.new message: "#{method} is undefined for #{self.class}"
    end

    def check_argument(list, value, attribute)
      unless list.include? value
        list.each{|l| l = "nil" if l.nil?}
        raise AlphaVantageRb::Error.new message: "Only #{list.join(", ")} are supported for #{attribute}",
    ...
    def check_datatype(datatype, file)
      if datatype == "csv" && file.nil?
        raise AlphaVantageRb::Error.new message: "No file specified where to save the CSV ata"
      elsif datatype != "csv" && !file.nil?
        raise AlphaVantageRb::Error.new message: "Hash error: No file necessary"
      end
    end
    ...
    def return_int_val(val, val_string, type="integer", check_positivity=true)
      ...
      if value.to_s != val
        AlphaVantageRb::Error.new message: "Error: #{val_string} is not a correct number"
      elsif check_positivity && value <= 0
        AlphaVantageRb::Error.new message: "Error: #{val_string} is not a correct positive #{type}"
      end
      return "&#{val_string}=#{val}"
    end
    ...
    def return_client(key, verbose=false)
      if key.is_a?(String)
        client = AlphaVantageRb::Client.new key: key, verbose: verbose
      elsif key.is_a?(AlphaVantageRb::Client)
        client = key
      else
        raise AlphaVantageRb::Error.new message: "Key should be a string"
      end
      return client
    end
